  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-8 border-l-4 border-l-orange-500 bg-white dark:bg-[#020617] border-slate-100 dark:border-white/5 relative overflow-hidden group">
        ...
        <Card className="p-8 border-l-4 border-l-purple-500 bg-white dark:bg-[#020617] border-slate-100 dark:border-white/5 relative overflow-hidden group">
           <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:scale-110 transition-transform">
              <Clock size={60} />
           </div>
      ...
      {/* Scoring Controller */}
      {scoringReport && (
        <Card className="p-10 border-orange-500/20 bg-orange-500/5 animate-in slide-in-from-top-6 duration-500 relative overflow-hidden">
           ...
           <div className="flex flex-col lg:flex-row items-center gap-10">
              ...
              <div className="flex items-center gap-8">
                ...
                <button 
                  onClick={handleScore}
                  className="px-10 py-5 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-2xl shadow-orange-500/30 active:scale-95 transition-all"
                >
                  Finalize Audit
                </button>
              </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-center justify-between bg-white dark:bg-[#020617] p-4 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-xl dark:shadow-2xl">
        ...
        <div className="flex gap-3 p-2 bg-slate-100 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-white/5">
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)} 
              className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-orange-500 text-white shadow-xl shadow-orange-500/20' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/5'}`}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-4">
        {filtered.map(report => (
          <Card key={report.id} hover className="p-8 group relative bg-white dark:bg-[#020617] border-slate-100 dark:border-white/5 overflow-hidden transition-all hover:border-orange-500/30">
            <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-10 transition-opacity">
               <TrendingUp size={40} className="text-orange-500" />
            </div>
            
            <div className="flex flex-col md:flex-row md:items-center gap-8">
              <div className="p-5 rounded-3xl bg-orange-500/10 dark:bg-orange-500/5 border border-orange-500/20 dark:border-orange-500/10 text-orange-600 dark:text-orange-500 group-hover:scale-110 transition-transform">
              ...
              <div className="flex items-center gap-8">
                ...
                {report.status === "Reviewed" ? (
                  ...
                  <button 
                    ...
                    className="px-8 py-4 bg-purple-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-purple-500/20 active:scale-95 transition-all"
                  >
                    Initiate Audit
                  </button>
                )}
